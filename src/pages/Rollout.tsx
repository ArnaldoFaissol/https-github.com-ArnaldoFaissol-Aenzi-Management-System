          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <Card className="lg:col-span-2 shadow-sm border-border/50">
              <CardHeader>
                ...
                <CardDescription>
                  Progresso calculado a partir do step atual de cada ativo no Kanban
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {(() => {
                  // Fases derivadas das 28 etapas (ACTIVATION_STEPS):
                  //   Fase 1 (Preparação): steps 1-9
                  //   Fase 2 (Instalação): steps 10-20
                  //   Fase 3 (Ativação):   steps 21-28
                  const phaseRanges = [
                    {
                      title: 'Fase 1: Preparação e Licenciamento',
                      desc: 'Alocação, vistoria, projeto, licenças e preparação do local (steps 1-9).',
                      min: 1,
                      max: 9,
                    },
                    {
                      title: 'Fase 2: Instalação e Comissionamento',
                      desc: 'Equipamentos, testes e comissionamento (steps 10-20).',
                      min: 10,
                      max: 20,
                    },
                    {
                      title: 'Fase 3: Ativação e Faturamento',
                      desc: 'FCU, contratos, ERP e pagamento (steps 21-28).',
                      min: 21,
                      max: 28,
                    },
                  ]
                  const phases = phaseRanges.map((p) => {
                    const inPhase = kanbanAssets.filter((a) => {
                      const n = Number(a.step_number)
                      return Number.isFinite(n) && n >= p.min && n <= p.max
                    }).length
                    const passed = kanbanAssets.filter((a) => {
                      const n = Number(a.step_number)
                      return Number.isFinite(n) && n > p.max
                    }).length
                    const total = kanbanAssets.length || 1
                    const progress = Math.round(((passed + inPhase * 0.5) / total) * 100)
                    return {
                      title: p.title,
                      desc: p.desc,
                      progress: Math.max(0, Math.min(100, progress)),
                      color: progress > 0 ? 'text-primary' : 'text-muted-foreground',
                    }
                  })
                  return phases
                })().map((phase, i) => (
            ...
            <Card className="bg-secondary text-secondary-foreground shadow-elevation border-none">
              ...
              <CardContent className="space-y-3">
                {(() => {
                  ...
                  kanbanAssets.forEach((a) => {
                    const uf = a.uf_code || 'N/D'
                    byUf[uf] = (byUf[uf] || 0) + 1
                  })
                  const sorted = Object.entries(byUf).sort((a, b) => b[1] - a[1]).slice(0, 8)
                  const total = kanbanAssets.length || 1
                  if (sorted.length === 0) {
                    return (
                      <p className="text-sm text-secondary-foreground/70">
                        Nenhum ativo com localização cadastrada ainda.
                      </p>
                    )
                  }
                  return sorted.map(([uf, count]) => {
